...
// confirms, before the Overworld is ever shown. Deliberately no checks or
// wellbeing calls — an ordinary-looking case, per the Saigon Constabulary
// lore note that Compact-level danger signals arrive later, not upfront.

VAR ledger = 0
VAR root = 0
VAR static = 0

The squad car pulls out of Cholon's press of noodle-stall steam and shopfront signage, three scripts deep on every storefront, and noses south toward District 4.
{ root >= 3:
...
The lab itself is nothing to look at — a low prefab bolted to the inside of the flood wall, no logo on the door, a single SEZAC cruiser already parked outside with its lights off. # background: avelineLabExterior
