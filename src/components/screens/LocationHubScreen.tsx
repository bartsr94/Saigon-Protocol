            ...
            {groupedInteractions.talk.map((presence) => {
              ...
              return (
                <button
                  ...
                  onClick={() => presence.available && enterStory(presence.storyLocationId)}
          ...
          <div className="grid gap-4 xl:grid-cols-[minmax(0,1fr)_24rem]">
            <div className="grid gap-4 md:grid-cols-2">
              {availableCharacters.map((presence) => {
                const npc = NPCS[presence.npcId]
                return (
                  <button key={presence.label} type="button" onClick={() => enterStory(presence.storyLocationId)} className="text-left">
              ...
              {availableActions.map((action) => (
                <button key={action.id} type="button" onClick={() => enterStory(action.storyLocationId)} className="text-left">
                  <Panel size="md" className="h-full p-4 transition-colors hover:!border-chrome-secondary hover:shadow-[0_0_18px_var(--color-chrome-secondary)]">
            ...
            <Panel size="lg" className="flex flex-col gap-4 p-5">
              ...
              <div className="flex flex-col gap-3">
                ...
                {availableCharacters.map((presence) => (
                  <CyberButton key={`${presence.label}-fallback`} onClick={() => enterStory(presence.storyLocationId)} tag="Talk">
                    {presence.label}
                  </CyberButton>
                ))}
                {availableActions.map((action) => (
                  <CyberButton key={`${action.id}-fallback`} onClick={() => enterStory(action.storyLocationId)} tag="Inspect">
                    {action.label}
                  </CyberButton>
                ))}
                {unavailableCharacters.map((presence) => (
              ...
              <div className="mt-auto border-t border-white/10 pt-4">
                <CyberButton onClick={handleReturnToMap}>Return to Map</CyberButton>
              </div>
            </Panel>
          </div>
        </div>
