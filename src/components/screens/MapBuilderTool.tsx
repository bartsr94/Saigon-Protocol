// Grid-authoring tool for Location Hub / District Street content
// (Architecture §7's Location Hub Layer, content/locationHubs.ts,
// content/districtStreets.ts). Deliberately has no fog-of-war, background
// art, or sprites — it only needs to produce the same `layoutRows` +
// `pois[]` shape those content modules already use, not look like the real
// screens. Output is plain JSON, hand-pasted into the content files rather
// than written directly — ids here are free text, not validated against the
// real HubId/LocationId/NpcId string-literal unions.
...
type BuilderMode = 'hub' | 'street'
type TileKind = 'wall' | 'floor' | 'void'
...
export function MapBuilderTool() {
  const [mode, setMode] = useState<BuilderMode>('hub')
  const [id, setId] = useState('')
  const [name, setName] = useState('')
  const [blurb, setBlurb] = useState('')
  const [backgroundId, setBackgroundId] = useState('')
  const [visionRadius, setVisionRadius] = useState('')
  const [grid, setGrid] = useState<TileKind[][]>(() => makeGrid(INITIAL_WIDTH, INITIAL_HEIGHT, 'void'))
  const [widthInput, setWidthInput] = useState(String(INITIAL_WIDTH))
  const [heightInput, setHeightInput] = useState(String(INITIAL_HEIGHT))
  const [pois, setPois] = useState<BuilderPoi[]>([])
  const [doors, setDoors] = useState<BuilderDoor[]>([])
  const [entryTile, setEntryTile] = useState<{ x: number; y: number } | null>(null)
    ...
    <div className="flex flex-1 gap-6 overflow-hidden">
      <div className="flex flex-1 flex-col gap-3 overflow-y-auto pr-2">
        ...
        <div className="grid grid-cols-2 gap-2">
          <input value={id} onChange={(e) => setId(e.target.value)} placeholder="id" className={INPUT_CLASS} />
